...
static int get_memory_mapping_mask(const EmuEnvState &emuenv, int gpu_idx) {
    int mask = (1 << 0);
...
static const QString show_live_area = QT_TR_NOOP(
    "Show the PS Vita Live Area screen before booting a game.");
...
static const QString gui_stylesheet = QT_TR_NOOP(
...
void SettingsDialog::load_config() {
...

    {
        ...
        const int gpu_idx = emuenv.cfg.gpu_idx;
        const int mask = get_memory_mapping_mask(emuenv, gpu_idx);
...
void SettingsDialog::setup_connections() {
    ...
    const std::vector<std::pair<QWidget *, QString>> desc_widgets = {
        ...
        { m_ui->show_live_area_screen, desc::show_live_area },
        ...
        { m_ui->combo_stylesheets, desc::gui_stylesheet },
        // Network
