...
TrophyCollectionDialog::TrophyCollectionDialog(EmuEnvState &emuenv, QWidget *parent)
    ...
    m_emuenv.np.trophy_state.add_trophy_unlock_callback(
        [weak_alive = std::weak_ptr<bool>(m_trophy_cb_alive), this](NpTrophyUnlockCallbackData &data) {
            ...
            QMetaObject::invokeMethod(this, [this, id = std::move(np_com_id)]() { on_trophy_unlocked(id); }, Qt::QueuedConnection);
        });
}
...
void TrophyCollectionDialog::load_icon_async(QStandardItemModel *model, int column, const QSize &icon_size, int row, const QString &path) {
    ...
    connect(watcher, &QFutureWatcher<QPixmap>::finished, this, [model, column, icon_size, row, watcher] {
...
void TrophyCollectionDialog::load_game_icon_async(int row, const QString &path) {
    load_icon_async(m_game_model, GC_Icon, m_game_icon_size, row, path);
}
...
void TrophyCollectionDialog::repaint_game_icons() {
    repaint_icons(m_game_model, GC_Icon, m_game_icon_size);
}
