...
// Include font.h first (which includes stb_truetype.h for declarations only)
...
bool font::is_bold_variant() const {
    return font_name.find("4.pvf") != std::string::npos;
}

static bool read_file_to_bytes(const std::string &path, std::vector<uint8_t> &out) {
...
font::font(std::string_view ttf_name, float size) {
    // Convert pt to px
...
glyph_load_setup font::get_glyph_files(language_class class_) const {
    ...
    case language_class::default_: {
        ...
        result.font_names.emplace_back(is_bold_variant() ? "ltn4.pvf" : "ltn0.pvf");
        // System fonts
...
codepage *font::initialize_codepage(char32_t c) {
    ...
    const auto try_load_font = [&](const std::string &file_path) -> bool {
        if (!read_file_to_bytes(file_path, bytes))
            return false;
