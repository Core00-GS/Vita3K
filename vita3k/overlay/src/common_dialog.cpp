...
std::unique_ptr<image_info> common_dialog_overlay::load_icon_white(const std::string &path) {
    auto img = std::make_unique<image_info>(path);
...
void common_dialog_overlay::ensure_icons_loaded() {
    if (m_icons_loaded)
        return;
    m_icons_loaded = true;
    m_cross_icon_data = load_icon_white("icons/cross.png");
    m_info_icon_btn_data = load_icon_white("icons/info.png");
    m_back_arrow_icon_data = load_icon_white("icons/doublearrow.png");
...
void common_dialog_overlay::build_message_dialog(DialogState &dialog) {
    ...
    m_message_label.set_font("ltn4.pvf", k_font_size);
    ...
    if (m_has_progress_bar) {
        ...
        m_progress_label.set_font("ltn4.pvf", k_font_size_small);
    ...
    if (msg.btn_num > 0) {
        ...
        for (uint8_t i = 0; i < msg.btn_num; i++) {
            ...
            btn.text.set_font("ltn4.pvf", k_font_size);
...
void common_dialog_overlay::build_savedata_list(DialogState &dialog, int date_format) {
    ...
    for (uint32_t i = 0; i < sd.slot_list_size; i++) {
        ...
        entry->title.set_font("ltn4.pvf", k_font_size);
        ...
        entry->date_label.set_font("ltn4.pvf", k_font_size_small);
        ...
        entry->subtitle.set_font("ltn4.pvf", k_font_size_small);
...
void common_dialog_overlay::build_savedata_fixed(DialogState &dialog, int date_format) {
    ...
    m_info_title.set_font("ltn4.pvf", k_font_size);
    ...
    m_info_date.set_font("ltn4.pvf", k_font_size_small);
    ...
    m_info_subtitle.set_font("ltn4.pvf", k_font_size_small);
...
void common_dialog_overlay::build_savedata_info(DialogState &dialog, int date_format) {
    ...
    if (sd.title.size() > sel && !sd.title[sel].empty())
        m_info_title.set_text(sd.title[sel]);
    m_info_title.set_font("ltn4.pvf", k_font_size_title);
    m_info_title.fore_color = k_color_text;
    m_info_title.back_color = { 0.f, 0.f, 0.f, 0.f };
    ...
    m_info_updated_label.set_font("ltn4.pvf", k_font_size);
    ...
    m_info_date.set_font("ltn4.pvf", k_font_size);
    ...
    m_info_details_key_label.set_font("ltn4.pvf", k_font_size);
    ...
    m_info_details.set_font("ltn4.pvf", k_font_size);
    ...
    back_btn.text.set_font("ltn4.pvf", k_font_size);
...
void common_dialog_overlay::build_ime_dialog(DialogState &dialog) {
    ...
    m_ime_text_label.set_font("ltn4.pvf", k_font_size);
...

    {
        ...
        submit.text.set_text(lang::get(lang::str::enter));
        submit.text.set_font("ltn4.pvf", k_font_size_small);
    ...
    if (m_ime_cancelable) {
        ...
        cancel.text.set_font("ltn4.pvf", k_font_size_small);
...
void common_dialog_overlay::build_trophy_setup(DialogState &dialog) {
    ...
    m_trophy_label.set_font("ltn4.pvf", k_font_size);
...
compiled_resource common_dialog_overlay::compile_savedata_list() {
    ...
    if (!m_list_title.empty()) {
        ...
        title_lbl.set_font("ltn4.pvf", k_font_size_title);
    ...
    if (m_save_slots.empty()) {
        ...
        no_saves.set_font("ltn4.pvf", k_font_size_title);
        ...
        for (int i = 0; i < static_cast<int>(m_save_slots.size()); i++) {
            ...
            const bool selected = (m_has_user_input && !m_cancel_btn_focused && i == m_selected_slot);
...

    {
        ...
        auto measure_substring = [&](const std::u32string &str) -> uint16_t {
            if (str.empty())
                return 0;
            overlay_element tmp;
            tmp.set_font("ltn4.pvf", k_font_size);
